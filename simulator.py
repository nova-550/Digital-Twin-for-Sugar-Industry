...
class SugarmillSimulator:
    def __init__(self):
        ...
        self.latest_state_physics: Dict[str, Dict[str, float]] = {}
    ...
    async def get_latest_snapshot(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "state_physics": self.latest_state_physics,
                "plant_kpis": [
    ...
    async def get_history(self, stage_id: str, param: str, n: int = 120) -> List[Dict]:
        """Returns historical time-series entries for Recharts."""
        async with self._lock:
        ...
        while self.is_running:
            try:
                async with self._lock:
                    ...
                    # 2. Propagate inputs through mass & energy balance physics (Standard Track - using ML if available)
                    outputs = {}
                    outputs["cane_handling"] = cane_handling_model(sampled["cane_handling"])
                    outputs["milling"] = milling_model(sampled["milling"], outputs["cane_handling"], force_physics=False)
                    outputs["clarification"] = clarification_model(sampled["clarification"], outputs["milling"])
                    outputs["evaporation"] = evaporation_model(sampled["evaporation"], outputs["clarification"])
                    outputs["crystallization"] = crystallization_model(sampled["crystallization"], outputs["evaporation"])
                    outputs["centrifugation"] = centrifugation_model(sampled["centrifugation"], outputs["crystallization"])

                    # 2b. Propagate inputs through Pure Physics (Physics Track - forcing physics in milling)
                    outputs_phys = {}
                    outputs_phys["cane_handling"] = outputs["cane_handling"]
                    outputs_phys["milling"] = milling_model(sampled["milling"], outputs["cane_handling"], force_physics=True)
                    outputs_phys["clarification"] = clarification_model(sampled["clarification"], outputs_phys["milling"])
                    
                    # Evaporator concentrations
                    ev_inputs_phys = {
                        "juice_brix_in_pct": outputs_phys["milling"].get("juice_brix_pct", 15.2),
                        "steam_flow_tph": sampled["evaporation"]["steam_flow_tph"],
                        "steam_economy": sampled["evaporation"]["steam_economy"]
                    }
                    outputs_phys["evaporation"] = evaporation_model(ev_inputs_phys, outputs_phys["clarification"])
                    outputs_phys["crystallization"] = crystallization_model(sampled["crystallization"], outputs_phys["evaporation"])
                    outputs_phys["centrifugation"] = centrifugation_model(sampled["centrifugation"], outputs_phys["crystallization"])

                    # 3. Fuse samples & outputs into atomic stage state packages
                    self.latest_state = {}
                    self.latest_state_physics = {}
                    for stage in DEFAULT_BASELINES:
                        # Combine raw sensors and calculated physical outputs
                        self.latest_state[stage] = {**sampled[stage], **outputs[stage]}
                        self.latest_state_physics[stage] = {**sampled[stage], **outputs_phys[stage]}
                    ...
                    for stage, data in self.latest_state.items():
                        for param, val in data.items():
                            ...
                            # Cap the in-memory window at 300 points
                            if len(self.history_buffers[key]) > 300:
                                self.history_buffers[key].pop(0)

                # 5. Log variables asynchronously to SQLite (runs out of critical lock path)
                for stage, data in self.latest_state.items():
                    # Log primary key metrics to save SQLite disk space
                    for key_param in ["imbibition_water_pct", "mill_extraction_pct", "juice_brix_pct", "estimated_ph", "juice_brix_out_pct", "supersaturation_coeff", "final_sugar_purity_pct"]:
...
import math  # Safety import for fluctuations math references
